package com.dmp.pansoft.flume.avroserializer;

import com.dmp.pansoft.flume.utils.DynamicAvroSchemaCreator;
...
public abstract class AbstractDynamicAvroSerializer implements EventSerializer, Configurable {
    ...
    protected abstract Map<String,Object> getOrderedData(Event event) throws Exception;
    @Override
    public void configure(Context context) {
        syncIntervalBytes = context.getInteger(SYNC_INTERVAL_BYTES, DEFAULT_SYNC_INTERVAL_BYTES);
        compressionCode = context.getString(COMPRESSION_CODEC, DEFAULT_COMPRESSION_CODEC);
    }
    ...
    public void write(Event event) throws IOException {
        try {
            ...
            Schema schema = schemaCreator.cretateSchema(orderedData);
            ...
            try {
                CodecFactory codecFactory = CodecFactory.fromString(compressionCode);
                dataFileWriter.setCodec(codecFactory);
            } catch (AvroRuntimeException e) {
                LOGGER.warn("Unable to instantiate avro codec with name (" + compressionCode + "). Compression disabled. Exception follows.", e);
            }
